...
> ⚠️ **신규 구축(시세 gRPC-only)에서는 이 broker fan-in 을 forwarder subscribe 허브로
> 대체한다 — `docs/price-ha-grpc.md` 참조.** 아래 broker 방식은 NH 레거시 interop 시에만
> 유효. 단 §1.2~2.2(결정성/archiver/quoteid/edge round_robin)는 gRPC-only 에서도 그대로 유효.
