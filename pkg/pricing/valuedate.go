...
//
// 휴일 캘린더 적용은 IsBusinessDayCal(d, cal) 사용. 본 함수는 backward compat
// 및 weekend-only 호출처용.
func IsBusinessDay(d time.Time) bool {
	return WeekendCalendar{}.IsBusinessDay(d)
}

// IsBusinessDayCal — 명시적 calendar 로 영업일 판정. cal == nil 이면 weekend-only.
func IsBusinessDayCal(d time.Time, cal Calendar) bool {
	if cal == nil {
		cal = WeekendCalendar{}
	}
...
// AddBusinessDays — weekend-only 캘린더로 영업일 추가/감산. backward compat.
func AddBusinessDays(d time.Time, n int) time.Time {
	return AddBusinessDaysCal(d, n, WeekendCalendar{})
}

// AddBusinessDaysCal — 명시적 calendar 로 영업일 추가/감산. n=0 이면 d 그대로
// (영업일 여부 무관). n>0 미래, n<0 과거. cal == nil 이면 weekend-only.
//
// 시각 부분은 보존되지 않으니 호출자가 startOfDay 등으로 미리 truncate 권장.
func AddBusinessDaysCal(d time.Time, n int, cal Calendar) time.Time {
	if n == 0 {
		return d
	}
	if cal == nil {
...
// BusinessDaysBetween — weekend-only 캘린더로 영업일 차이. backward compat.
func BusinessDaysBetween(from, to time.Time) int {
	return BusinessDaysBetweenCal(from, to, WeekendCalendar{})
}
...
// SpotDate — 거래일 (now) 기준 SPOT 결제일 (weekend-only). backward compat.
func SpotDate(now time.Time, spotDays int) time.Time {
	return SpotDateCal(now, spotDays, WeekendCalendar{})
}

// SpotDateCal — 명시적 calendar 로 SPOT 결제일. 일반적으로 T+2 (영업일).
// 일부 통화쌍 (USD/CAD 등) 은 T+1. spotDays 인자로 명시.
func SpotDateCal(now time.Time, spotDays int, cal Calendar) time.Time {
	if spotDays < 0 {
		spotDays = 0
	}
	return AddBusinessDaysCal(startOfDay(now), spotDays, cal)
}
