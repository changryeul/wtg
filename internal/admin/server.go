...
func (s *Server) Start(ctx context.Context) error {
	...
	// 스왑포인트 화면 — 수신+delta+effective 조회 + delta 조정 적용 (same-origin proxy → mci-price).
	mux.HandleFunc("GET /v1/admin/swap", UpstreamProxy(s.cfg.PriceURL, "/v1/pricing/swap", s.logger))
	mux.HandleFunc("POST /v1/admin/swap-apply", UpstreamProxy(s.cfg.PriceURL, "/v1/pricing/swap-delta", s.logger))
	...
	mux.HandleFunc("GET /v1/admin/mci-health", MciHealth(s.cfg.MciHealthTargets, s.cfg.EtcdEndpoints, s.cfg.EnableProcessControl))
	// MCI 프로세스 제어 (start/stop/restart) — sudo systemctl. 기본 off, allowlist
	// (etcd/mci-admin 제외) + audit. --enable-process-control + sudoers 선행.
	mux.HandleFunc("POST /v1/admin/mci/{name}/{action}", MciControl(&MciControlDeps{
		Logger:  s.logger,
		Audit:   s.audit,
		Enabled: s.cfg.EnableProcessControl,
	}))
	...
	// 차트 통계 — mci-chart 의 누적 카운터 + Hub state. ws 클라이언트 health.
	mux.HandleFunc("GET /v1/admin/chart-stats", UpstreamProxy(s.cfg.ChartURL, "/v1/chart-stats", s.logger))
