...
func main() {
	...
	// swap store (SHM fold 의 swap 파트 대체, in-memory). received=로이터 수신
	// (POST /v1/pricing/swap-received), delta=운영자 조정(POST /v1/pricing/swap-delta).
	// effective=received+delta (add 규약) 를 forward/algo 가 사용.
	swapStore := price.NewSwapStore()
	srv.AttachSwapStore(swapStore)
	...
	if grpcSrv != nil && cfg.AlgoStreamEnabled {
		...
		// forward tenor 별 effective(received+delta) swap 합성 (mds fold 대응).
		algoSrv.SetSwapProvider(price.NewSwapProvider(swapStore, symbols))
		logger.Info("AlgoStream 활성 — SubscribeAlgo (swap provider 주입)",
