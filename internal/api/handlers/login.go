...
func Login(deps *Deps) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		frame := &mymq.FrameInput{
			...
			Body: loginBody(deps, routingKey, channel, req),
...
func loginBody(deps *Deps, routingKey, channel string, req LoginRequest) []byte {
	if body, _, err := wireBuildBody(deps.SvcIO, routingKey, "", channel, req.Header, req.Data); err == nil && body != nil {
		return body
	}
