...
| 검증 | `mock-lp` | — | — | **시나리오형 mock LP** — LP(SMB/KMB/EBS/CMB)별 **결정적** 호가/체결(FIX 269=2)을 UDP 35=W 로 송신. per-source→BestConsumer→cross→AlgoQuote(last/mid/source) 경로를 값까지 결정적 e2e 검증 (load-gen 은 랜덤 부하, mock-lp 는 시나리오 검증). `--scenario`/`--once`/`--interval` |
