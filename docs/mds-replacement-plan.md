...
| MES 주문 전송 (trn → mat) | `IBA_MES_ORD_QUE_PUSH` → MESORDSQUE(SHM큐) → wfa → TCP :26003 → `mat_rcv` | **MyMQ broker 큐** — trn 이 `mymq_send` (exchange MES / rkey ORD, FC_NOTIFY) → broker 큐 → mat 이 AP 로 수신. **TCP 직결안 기각 (2026-07-14)**: 큐의 존재 이유가 안정성 (mat 순단 시 버퍼링·발신 논블로킹) 이므로 큐는 큐로 대체. trn 은 MyMQ 핸들 기보유, mat 도 `-lmymq` 기의존 (`mat_ord/sender`) — 신규 의존 0, 주문/체결은 저빈도라 broker 부하 (시세 폭주 이슈) 무관 | 설계 확정 — 구현: trn 치환(소형) + mat 수신 어댑터 + 큐/바인딩 선언 |
| MES 체결 수신 (mat → trn측) | `mat_snd` → TCP :26004 → wfa → MESEXERQUE(SHM큐) → `IBA_MES_EXE_QUE_POP` (WD300002) | **MyMQ broker 큐** — mat 이 publish (exchange MESEXE) → WD300002 가 구독 수신 (mon 은 mymq 기반) | 설계 확정 — 동일 |
