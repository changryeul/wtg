...
| `-idempotency`, `-idempotency-ttl`              | —                                | `Idempotency-Key` 헤더 처리 활성 (def off) / reservation TTL (def 5m). 활성 시 중복 매매 차단 (더블 클릭 / 모바일 retry / 멀티 인스턴스). backend 는 `-redis` 가 채워지면 Redis (다중 인스턴스 공유 reservation), 비면 Memory | 운영 권장                   |
| `-idempotency-redis-prefix`                     | —                                | Idempotency Redis key prefix (default `wtg:idem:`). `-redis` 활성 시 sessions / refresh 와 분리                                                                                         | 운영                      |
| `-login-mode`                                   | `WTG_API_LOGIN_MODE`             | `legacy`(기본) = 단일 LOGON + cookie_t. `chain` = 엔진 인증 사슬 (W1101S02→W1130A02, lgnIdntCon 보관). chain 은 `-svc-inc-dir` **필수** (부팅 시 검증)                                                  | NH 엔진 배포 시 `chain`      |
| `-login-cert-alias`, `-login-session-alias`, `-login-logout-alias` | —             | chain 단계별 alias override (기본 W1101S02 / W1130A02 / W1130A03). 라우팅 룰 미등록 시 `dom/<alias>` fallback                                                                                  | 기본값이면 생략                |
| `-login-skip-cert`                              | `WTG_API_LOGIN_SKIP_CERT`        | chain 에서 ① 인증서 인증(W1101S02) 건너뛰고 `data.lgnId` 로 세션만 개설. 사용자는 CSC004M 존재 필요. `-login-mode=chain` 에서만 유효                                                                            | 인증서 미적용 과도기, **운영 금지** |
...
**alias × tier 메트릭** (`GET /v1/admin/alias-stats`): /v1/tx 와 /v1/tx/bulk 의
호출을 alias 와 tier 조합으로 누적. 같은 alias 의 VIP / GOLD / STD 별 latency /
error_rate 분리 관찰 가능 — 특정 tier 의 매매 품질 저하 식별.

응답 row 형식: `{alias, tier, calls, errors, avg_latency_ms, max_latency_ms,
error_rate_pct, last_call_unix}`. UserProfileResolver 비활성 (degraded) 의
빈 tier 는 `__notier__`, alias 없는 raw 호출은 `__raw__`. Calls 내림차순 정렬,
동률은 alias / tier 사전순.

**BulkTx** (`POST /v1/tx/bulk`): 다건 매매를 한 HTTP 요청에 묶어 처리.
- request: `{"items":[envelope1, envelope2, ...], "stop_on_error":false}`
- response: `{"items":[{status, envelope|error+message}, ...]}` — request items[i]
  와 1:1 대응.
- 최대 100건/요청. 더 큰 묶음은 chunk 분할.
- 순차 처리 (ordering 보장). 각 item 별 정책 검사 + alias 메트릭 + OTel child span.
- `stop_on_error=true` 시 첫 실패 후 나머지 `error="not_attempted"`. default false.
- Idempotency-Key 는 bulk 전체 단위 — 같은 키 + 같은 전체 body 재요청 시 캐시.
...
- **`docs/observability.md`** — 운영 진단/관측 가이드 (silent 가드 / metric /
  endpoint / admin UI 5 페이지). "떠 있는 상태에서 무엇을 보는가" 한 곳 정리.
- `docs/admin-ui-test-guide.md` — mci-admin UI 페이지별 동작/테스트 시나리오 (21 페이지)
