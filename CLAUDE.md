...
| Internal | `mci-api` | 8080 | `ChannelWeb` | `/v1/tx` `/v1/login` (Site/Tier → Session.Profile + JWT) `/v1/refresh`. `data` 가 JSON 이면 svc I/O 명세(`pkg/svcio`) 기반 고정폭 전문 자동 조립. **`--login-mode=chain`** 시 `/v1/login` 이 엔진 인증 사슬 (W1101S02→W1130A02, lgnIdntCon 보관) 오케스트레이션 — `docs/auth.md` §3.1 |
...
- `docs/quoteid-validation-rfc.md` — quote ID 검증 RFC (sync/async 모드, store 선택, 성능 트레이드오프)
 (mds UDP APSISE+SHM MDFOLD → WTG SubscribeAlgo/AlgoQuote). fillprc/mid/excode gap 분석 + refprctype 별 이관 판정
