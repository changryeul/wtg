...
| DMZ | `mci-edge-fix-ord` | 5001 (FIX) / 5002 (stats) | (HTTP forward) | FIX 4.4 session 종단 + Logon 검증 + NewOrderSingle (35=D) → `/v1/tx` alias. 정적 counterparty seed (Phase A) — etcd watch 는 Phase B |
| DMZ | `mci-edge-fix-md` | 5011 (FIX MD) / 5012 (stats) | (gRPC client) | FIX 4.4 시세 gateway — 외부 카운터파티 MarketDataRequest (35=V) → MarketDataSnapshotFullRefresh (35=W) 응답 |
| DMZ | `mci-edge-tcp` | 5021 (raw TCP) / 5022 (stats) | (HTTP forward) | 레거시 cs (HTS/EMP) 용 raw TCP 전문 gateway — 지속 연결 + 4B length-prefix 전문 + 빈 프레임 heartbeat (mymq 컨벤션) → `/v1/tx` raw 모드. admin "TCP 게이트웨이" 페이지가 stats 소비 (`/v1/admin/tcp-gw/stats` proxy). 검증은 `cmd/tcp-tester` |
| Internal | `mci-api` | 8080 | `ChannelWeb` | `/v1/tx` `/v1/login` (Site/Tier → Session.Profile + JWT) `/v1/refresh`. `data` 가 JSON 이면 svc I/O 명세(`pkg/svcio`) 기반 고정폭 전문 자동 조립. **`--login-mode=chain`** 시 `/v1/login` 이 엔진 인증 사슬 (W1101S02→W1130A02, lgnIdntCon 보관) 오케스트레이션, `--login-skip-cert` 로 인증서 단계 생략 (data.lgnId 로 세션 개설) — `docs/auth.md` §3.1 |
...
| 검증 | `quote-replay` | — | — | mds `.trc` 실 캡처 재생기 — 35=W/X 를 원본 간격(배속 옵션)으로 UDP 재송신, 콤마 구분 다중 목적지로 mds/WTG 동시 입력. mds 전환 dual-run 게이트 도구 (`docs/mds-replacement-plan.md` Stage 0) |
...
etc/                     # 운영 설정 (symbols.json, profiles.json, pricing.json, sql/, ...)
  db-mirror/             # fx-sync 미러 JSON (currency/pair/swap_point/user_profile/
                         # customer_margin/hq_margin/site_margin) — 외환 운영 DB 마스터의 etcd seed 원본
```
...
### 3. 시세 raw (UDP → mci-price, gRPC-only 기본)
`quote-forwarder` 가 UDP FIX 4.4 (35=W/X) 파싱 → v1 envelope 발행. **신규 구축 기본은 gRPC-only** (`--publish-mode grpc`, mci-price PublishTick 직접 — 하류 SubscribeQuote 와 일관). broker(`FCCast/SubBroadcast`, Exchange="PRICE")는 **NH 레거시 mymqd interop 전용**:
...
- `docs/algo-quote-reconciliation.md` — algo 시세 경로 대사표 (mds UDP APSISE+SHM MDFOLD → WTG SubscribeAlgo/AlgoQuote). fillprc/mid/excode gap 분석 + refprctype 별 이관 판정
- `docs/reuters-swap-adapter-spec.md` — 로이터 swap 피드 어댑터 스펙 초안 (mds onrecv_swap/swaprate_t UDP wire → POST /v1/pricing/swap-received). 전송방식 확정 시 구현. 주입 seam·effective(received+delta)는 완성
...
- `docs/customer-spread-sync.md` — 고객별 스프레드 pricing (customer spread → CustomerMargin override, tier 대체 + 미등록 tier fallback). fx-sync SyncCustomerMargins, 엔진·런타임 무변경
...
- `docs/mock-lp-guide.md` — mock-lp 로컬 시세 검증 가이드 (자동 verify 스크립트 + 수동 4-터미널 순서 + per-source/forward swap/cross + 트러블슈팅)
- `docs/push-test-guide.md` — mci-push unsolicited push 로컬 검증 (Track B HTTP push, mci-push --no-broker + websocat ws + curl/cside producer + dev X-WTG-User vs 운영 JWT 신원 흐름)
...
- `docs/price-ha-grpc.md` — 시세 gRPC-only 다중 인스턴스 HA. forwarder subscribe 허브(fan-in), dual-active+dedup, warm-up gate, edge round_robin. 코어 HA 는 mci-price-ha.md 유효
