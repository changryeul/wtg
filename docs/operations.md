...
| `-ip-rate`, `-ip-burst` | — | IP 단위 rate limit (def 100 TPS / burst 200, 0=비활성) |
...
| `-appl`, `-instance` | broker 등록 ApplName (def `quote-fwd` / 1) |
| `-include-fix` | true 면 envelope 에 raw FIX 도 포함 (감사용) |
| `-metrics` | Prometheus + `/stats` HTTP listen (예: `127.0.0.1:9091`) |
