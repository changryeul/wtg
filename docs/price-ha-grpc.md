...
| 컴포넌트 | 상태 |
|---|---|
| forwarder 팬아웃 허브 core (Registry + Broadcast + slow evict) | ⬜ 스캐폴딩 (edge Registry 패턴 재사용) |
| proto `TickService.SubscribeTicks` | ⬜ |
| forwarder gRPC 서버 배선 + `--publish-mode hub` | ⬜ |
| mci-price `TickSubscriber` client + reconnect + (source,seq) dedup | ⬜ |
| warm-up gate + `/v1/ready` | ⬜ |
| e2e (forwarder→N mci-price→edge failover) | ⬜ |

각 단계 TDD. proto/gRPC 배선은 core 검증 후.
