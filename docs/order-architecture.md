...
| NDF | JPN | 227.10.40.33 | 45010 |
| NDF | DUN | 227.10.40.34 | 45010 |

> **NDF LP**: `JPN`(=JPM NDF) / `DUN`(=DUH NDF) 는 NDF(차액결제선물환) 피드. category=`ndf`
> 로 별도 그룹핑 (`/v1/sources` 가 UI 에서 spot 중계사와 분리 노출). 4-분류: broker / bank /
> foreign / ndf.
